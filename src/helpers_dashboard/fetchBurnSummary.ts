import GGWBurnManagerJson from "@/abi/GGWBurnManager.json"
import Web3 from 'web3'
import { Interface as AbiInterface } from '@ethersproject/abi'
import { GET_CHAIN_RPC } from '@/web3/chains'
import getMultiCall, { getMultiCallAddress, getMultiCallInterface }from '@/web3/getMultiCall'

import { callMulticall } from '@/helpers/callMulticall'
import Web3ObjectToArray from "@/helpers/Web3ObjectToArray"
import { fromWei } from '@/helpers/wei'

const fetchBurnSummary = (options) => {
  const {
    address,
    chainId,
  } = {
    ...options
  }

  return new Promise((resolve, reject) => {
    const GGWBurnManagerAbi = GGWBurnManagerJson.abi

    const multicall = getMultiCall(chainId)
    const abiI = new AbiInterface(GGWBurnManagerAbi)

    callMulticall({
      multicall,
      target: address,
      encoder: abiI,
      calls: {
        burnSummary:            { func: 'getSummaryInfo' },
      }
    }).then((mcAnswer) => {
      
      resolve({
        chainId,
        address,
        ...mcAnswer,
      })
      console.log('>>> summary', mcAnswer)
    }).catch((err) => {
      console.log('>>> Fail fetch all info', err)
      reject(err)
    })
  })
}

export default fetchBurnSummary