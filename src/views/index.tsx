...
function GGWDashboardView(pageProps) {
  ...
  let stakePoolLoad = 0
  if (stakeInfo && stakeInfoFetched) {
    stakePoolLoad = new BigNumber(
      stakeInfo.depositsAmount
    ).dividedBy(
      toWei(STAKE_TARGET, stakeInfo.tokenDecimals)
    ).toFixed(10)
  }
  return (
    <>
        ...
        <section>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-4 mb-4">
            ...
            <div className="gradient-border rounded-xl p-6 card-hover bg-gradient-to-br from-purple-900/20 to-slate-900/50">
              <div className="flex items-center gap-3 mb-4">
                ...
                <div>
                  ...
                  <div className="text-3xl font-bold text-white">
                    {(stakeInfo && stakeInfoFetched) ? (
                      <>{fromWei(stakeInfo.depositsAmount, stakeInfo.tokenDecimals)}</>
                    ) : (
                      <LoadingIndicator text={`...`} circle={false} color={``} />
                    )}
                    {` `}
              ...
              <div className={`space-y-2 ${(stakeInfo && stakeInfoFetched) ? '' : 'animate-pulse'}`}>
                ...
                <div className="h-2 bg-slate-800 rounded-full overflow-hidden">
                  <div 
                    ...
                    style={{ width: `${stakePoolLoad}%` }}
                  ></div>
                </div>
                <div className="flex justify-between text-xs text-slate-500">
                  <span>
                    {(stakeInfo && stakeInfoFetched && tokenPriceFetched) ? (
                      <>
                        {`$`}
                        {
                          new BigNumber(
                  ...
                  <span>Target: {STAKE_TARGET} GGC</span>
                </div>
              </div>
            </div>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-6">
            ...
            <div className="space-y-3">
              ...
              <div className="bg-slate-900/30 border border-slate-800 rounded-xl p-4 flex justify-between items-center group hover:border-purple-500/30 transition-colors">
                ...
                <div className="flex items-center gap-3">
                  <code className="text-xs mono text-slate-400 bg-slate-800 px-2 py-1 rounded">
                    {getShortAddress(STAKE_CONTRACT)}
                  </code>
                  <button
                    onClick={() => {
                      copyToClipboard(STAKE_CONTRACT)
                      addNotification('success', 'Address of Stake contract copied')
                    }}
                    className="text-slate-500 hover:text-white transition-colors"
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                  ...
                  <a
                    href={getAddressLink(chainId, STAKE_CONTRACT)}
                    target="_blank"
                    className="text-slate-500 hover:text-white transition-colors"
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <div className="space-y-3">
              ...
              <div className="bg-slate-900/30 border border-slate-800 rounded-xl p-4 flex justify-between items-center group hover:border-purple-500/30 transition-colors">
                ...
                {(depositsInfo && depositsInfoFetched) ? (
                  <div className="flex items-center gap-3">
                    <code className="text-xs mono text-slate-400 bg-slate-800 px-2 py-1 rounded">
                      <>{getShortAddress(depositsInfo.gasAddress)}</>
                    </code>
                    <button 
                      onClick={() => {
                        copyToClipboard(depositsInfo.gasAddress)
                        addNotification('success', 'Address of Gas Payer copied')
                      }}
                      className="text-slate-500 hover:text-white transition-colors"
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                    ...
                    <a 
                      href={getAddressLink(chainId, depositsInfo.gasAddress)}
                      target="_blank"
                      className="text-slate-500 hover:text-white transition-colors"
                    >
                      <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
